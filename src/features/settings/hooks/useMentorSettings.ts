...
export function useMentorSettings() {
  ...
  const fetchMentorSettings = useCallback(async () => {
    ...
    try {
      if (!user) {
        setMentorId(null);
        setFormData(initialMentorSettingsFormData);
      ...
      const [languagesResult, expertiseResult, availabilityResult] =
        await Promise.all([
          supabase
      ...
      if (languagesResult.error || expertiseResult.error || availabilityResult.error) {
        setFetchError("Failed to load mentor settings");
        return;
      }
      ...
      console.error("fetchMentorSettings unexpected error", error);
      setMentorId(null);
      setFormData(initialMentorSettingsFormData);
      setFetchError("Failed to load mentor settings");
    } finally {
